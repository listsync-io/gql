...
class DataclassesRenderer:
    ...
    def render_shared_code(self):
        ...
        buffer.write("class BaseOperation:")
        buffer.write("    data: Any = None")
        buffer.write("    errors: Any = None")
        buffer.write("")
        buffer.write("    @classmethod")
        buffer.write("    def execute(")
        buffer.write("        cls,")
        buffer.write("        *args,")
        buffer.write(
            "        on_before_callback: Callable[[Mapping[str, str], Mapping[str, str]], None] = None,"
        )
        buffer.write("        **kwargs")
        buffer.write("    ):")
        buffer.write(
            "        client = Client('http://localhost:8080/v1/graphql', auth.user().get_hasura_header())"
        )
        buffer.write(
            "        variables = {key: value for key, value in zip(cls.__QUERY_PARAMS__, args)}"
        )
        buffer.write("        variables.update(kwargs)")
        buffer.write(
        ...
        buffer.write("        return (")
        buffer.write("            DataclassesRenderer.relayify(data, snake_case_str)")
        buffer.write("            if auth.is_standard_call()")
        buffer.write(
            "            else DataclassesRenderer.simplify(data, snake_case_str)"
        )
        buffer.write("        )")
        buffer.write("")
        buffer.write("    @classmethod")
        buffer.write("    async def execute_async(")
        buffer.write("        cls,")
        buffer.write("        *args,")
        buffer.write(
            "        on_before_callback: Callable[[Mapping[str, str], Mapping[str, str]], None] = None,"
        ...
        buffer.write("    ):")
        buffer.write(
            "        client = AsyncIOClient('http://localhost:8080/v1/graphql', auth.user().get_hasura_header())"
        )
        buffer.write(
            "        variables = {key: value for key, value in zip(cls.__QUERY_PARAMS__, args)}"
        )
    ...
    def render(self, parsed_query: ParsedQuery, operation_type: str):
        buffer = CodeChunk()
        ...
        for obj in sorted_objects:
            if isinstance(obj, ParsedObject):
                self.__render_object(parsed_query, buffer, obj, operation_type)
            elif isinstance(obj, ParsedOperation):
                self.__render_operation(parsed_query, buffer, obj, operation_type)
    ...
    def __render_object(
        ...
        op_name = obj.name
        if operation_type in obj.name:
            op_name = operation_type + "Data"

        with buffer.write_block(f"class {op_name}{class_parents}:"):
            # render child objects
            for child_object in obj.children:
                self.__render_object(parsed_query, buffer, child_object, operation_type)
    ...
    def __render_operation(
        ...
        buffer.write("@dataclass_json")
        buffer.write("@dataclass")
        with buffer.write_block(f"class {operation_type.capitalize()}(BaseOperation):"):
            ...
            buffer.write(f"__QUERY_PARAMS__ = [{variables_dict_str}]")
            ...
            # operation fields
            buffer.write("")
            buffer.write(f"data: {operation_type}Data = None")
            buffer.write("errors: Any = None")
            buffer.write("")
